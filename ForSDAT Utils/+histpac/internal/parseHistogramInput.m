...
% functions.
    ...
    parser.addOptional('ModelFittingMode', '', @(mfm) mustBeMember(mfm, {'data', 'frequencies'}));
    
    % Plotting options
    parser.addOptional('PlotTo', [], ...
        @(h) assert(isgraphics(h) && (isa(h, 'matlab.ui.Figure') || isa(h, 'matlab.graphics.axis.Axes') || isa(options.PlotTo, 'matlab.ui.control.UIAxes')),...
